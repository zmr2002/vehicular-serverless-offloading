...
param(
    ...
    [string]$Config = 'configs\hybrid-capacity-paper-scale.toml',
    [switch]$DryRun
)
...
if (-not $DryRun -and $status.Count -gt 0) {
    throw 'Commit or remove working-tree changes before running the resumable sweep.'
}

$arguments = @(
    (Join-Path $repo 'scripts\run-frozen-hybrid-capacity-sweep.py'),
    '--config',
    $configPath
)
if ($DryRun) {
...
& $python @arguments
if ($LASTEXITCODE -ne 0) {
    throw "Hybrid capacity sweep exited with code $LASTEXITCODE."
}
